class SkillCategoriesController < ApplicationController
  ...
  before_action :authenticate_user!, only: [:all_stuff]
  ...
  def all_stuff
    ...
    @all_skills=@skills.as_json(include:{skills:{only: ['id','name','description']}}, only: ['name'])
    @user= current_user
    @user_skills = @user.skills.select('skills.*,user_skills.*')
    @user_skills = @user.add_skills_info(@user_skills)
  end
  private
